...
لغو تسک‌ها بخش مهمی از کنترل جریان در برنامه‌های async هست. بدونش ممکنه برنامه‌ت بی‌هدف به اجرای کارهایی ادامه بده که دیگه به درد نمی‌خورن. با درک درست از لغو و استفاده از `CancelledError` و بخش `finally`، می‌تونی برنامه‌هات رو هم تمیزتر و هم قابل‌کنترل‌تر بنویسی.
